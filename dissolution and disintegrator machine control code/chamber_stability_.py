...
if __name__== "__main__":
    ...
    for i in range(1, argc): # ignore first argument which is command name
      g = int(sys.argv[i])
      if (g >= 100):
         s = chamber_stability_.sensor(pi, g-100, callback=callback)
      else:
         s = chamber_stability_.sensor(pi, g)
    ...
    while True:
      try:
         for s in S:
            if s[0] >= 100:
               k = s[1].read()
               if not(k[3] >= 25) and not(k[4] >= 80.0):
                   GPIO.output(heater, GPIO.HIGH)
                   GPIO.output(humidifier, GPIO.HIGH)
                   GPIO.output(compressor, GPIO.LOW  )
               elif not(k[3] >= 25) and k[4]>=80.0:
                   if k[4] == 80.0:
                       GPIO.output(compressor, GPIO.LOW)
                   GPIO.output(heater, GPIO.HIGH)
                   GPIO.output(humidifier, GPIO.LOW)
                   GPIO.output(compressor, GPIO.HIGH)
               elif k[3] >= 25 and not(k[4] >= 80.0):
                   if k[3] == 25:
                       GPIO.output(compressor, GPIO.LOW)
                   GPIO.output(heater, GPIO.LOW)
                   GPIO.output(humidifier, GPIO.HIGH)
                   GPIO.output(compressor, GPIO.HIGH)
               elif k[3] >= 25 and k[4] >= 80.0:
                   if k[3] == 25 or k[4] == 80.0:
                       GPIO.output(compressor, GPIO.LOW)
                   GPIO.output(heater, GPIO.LOW)
                   GPIO.output(humidifier, GPIO.LOW)
                   GPIO.output(compressor, GPIO.HIGH)
                   
            # values displayed by callback
            else:
               d = s[1].read()
               if not(d[3] >= 25) and not(d[4] >= 80.0):
                   GPIO.output(heater, GPIO.HIGH)
                   GPIO.output(humidifier, GPIO.HIGH)
                   GPIO.output(compressor, GPIO.LOW  )
               elif not(d[3] >= 25) and d[4]>=80.0:
                   if d[4] == 80.0:
                       GPIO.output(compressor, GPIO.LOW)
                   GPIO.output(heater, GPIO.HIGH)
                   GPIO.output(humidifier, GPIO.LOW)
                   GPIO.output(compressor, GPIO.HIGH)
               elif d[3] >= 25 and not(d[4] >= 80.0):
                   if d[3] == 25:
                       GPIO.output(compressor, GPIO.LOW)
                   GPIO.output(heater, GPIO.LOW)
                   GPIO.output(humidifier, GPIO.HIGH)
                   GPIO.output(compressor, GPIO.HIGH)
               elif d[3] >= 25 and d[4] >= 80.0:
                   if d[3] == 25 or d[4] == 80.0:
                       GPIO.output(compressor, GPIO.LOW)
                   GPIO.output(heater, GPIO.LOW)
                   GPIO.output(humidifier, GPIO.LOW)
                   GPIO.output(compressor, GPIO.HIGH)
